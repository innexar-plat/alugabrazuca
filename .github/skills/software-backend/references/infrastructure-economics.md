...
const calculateSLAPenalty = (
  ...
  const penaltyPercent = Math.min(
    missedPoints * config.penaltyPercentPerPoint,
    100,
  );
...
const budgetAlerts: BudgetAlert[] = [
  ...
  { threshold: 80, action: "warn", recipients: ["team-slack", "tech-lead"] },
  {
    threshold: 100,
    action: "escalate",
    recipients: ["team-slack", "tech-lead", "finance"],
  },
...
- [ ] Total spend vs. budget: \_\_\_\_% of monthly budget
- [ ] Week-over-week change: \_\_\_\_% (flag if >10%)
...
- [ ] Idle resources to terminate: **\*\***\_\_\_\_**\*\***
- [ ] Over-provisioned instances: **\*\***\_\_\_\_**\*\***
- [ ] Unattached volumes/IPs: **\*\***\_\_\_\_**\*\***
...
const attributeCosts = (
  ...
  const totalWeight = features.reduce(
    (sum, f) => sum + f.requestShare * f.computeWeight * f.storageWeight,
    0,
  );
  ...
  return features.map((f) => ({
    ...
    attributedCost:
      (totalMonthlyCost *
        (f.requestShare * f.computeWeight * f.storageWeight)) /
      totalWeight,
  }));
};
