...
app.post("/api/upload-svg", upload.single("svg"), async (req, res) => {
  try {
    ...
    if (
      file.originalname.includes(" .svg") ||
      /\s+\.svg$/i.test(file.originalname)
    ) {
  ...
  if (claimedMime.startsWith("image/")) {
    ...
    try {
      ...
      if (pixelCount > 100000000) {
        // 100 megapixels
        throw new ValidationError(
          "Image too large (possible decompression bomb)",
        );
      }
    } catch (error) {
...
const convertImage = async (inputPath) => {
  ...
  await sharp(safePath).resize(800, 600).toFile("/output/converted.png");
};
...
const validator = {
  ...
  string: (
    value,
    { minLength = 0, maxLength = Infinity, pattern = null } = {},
  ) => {
  ...
  array: (
    value,
    { minLength = 0, maxLength = Infinity, itemValidator = null } = {},
  ) => {
