...
describe('AuthService', () => {
  ...
  describe('login', () => {
    ...
    it('should throw UnauthorizedException when user not found', async () => {
      ...
      await expect(
        service.login({ email: 'wrong@example.com', password: 'Test@1234' }, '127.0.0.1'),
    ...
    it('should throw UnauthorizedException when password is wrong', async () => {
      ...
      await expect(
        service.login({ email: 'test@example.com', password: 'WrongPass@1' }, '127.0.0.1'),
      ).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('should throw ForbiddenException when email not verified', async () => {
      ...
      await expect(
        service.login({ email: 'test@example.com', password: 'Test@1234' }, '127.0.0.1'),
  ...
  describe('verifyEmail', () => {
    it('should verify email successfully', async () => {
      mockJwt.verify.mockReturnValue({ sub: mockUser.id, type: 'email_verification' });
      mockPrisma.user.findUnique.mockResolvedValue({ ...mockUser, emailVerified: false });
    ...
    it('should throw NotFoundException when user not found', async () => {
      mockJwt.verify.mockReturnValue({ sub: 'nonexistent', type: 'email_verification' });
      mockPrisma.user.findUnique.mockResolvedValue(null);
  ...
  describe('googleLogin', () => {
    ...
    it('should link Google to existing email account', async () => {
      mockPrisma.user.findUnique.mockResolvedValue({ ...mockUser, provider: 'email', providerId: null });
      mockPrisma.user.update.mockResolvedValue(mockUser);
      mockPrisma.refreshToken.create.mockResolvedValue({});
  ...
  describe('updateProfile', () => {
    ...
    it('should sanitize bio field to prevent XSS', async () => {
      mockPrisma.user.update.mockResolvedValue({ ...mockUser, bio: '&lt;script&gt;alert()&lt;/script&gt;' });

      await service.updateProfile(mockUser.id, { bio: '<script>alert()</script>' });
  ...
  describe('changePassword', () => {
    it('should change password successfully', async () => {
      mockPrisma.user.findUnique.mockResolvedValue({ passwordHash: '$2b$12$hashedPassword' });
      mockPrisma.user.update.mockResolvedValue({});
