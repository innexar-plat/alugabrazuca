...
describe('InquiryService', () => {
  ...
  describe('create', () => {
    ...
    it('should create an inquiry successfully', async () => {
      ...
      mockPrisma.inquiry.create.mockResolvedValue({ ...mockInquiry, listing: {}, tenant: {}, host: {} });
    ...
    it('should throw BadRequestException when listing is not active', async () => {
      mockPrisma.listing.findUnique.mockResolvedValue({ ...mockListing, status: ListingStatus.paused });
    ...
    it('should throw BadRequestException when host tries to inquire own listing', async () => {
      mockPrisma.listing.findUnique.mockResolvedValue(mockListing);
      await expect(service.create(HOST_ID, dto)).rejects.toThrow(BadRequestException);
    });
  ...
  describe('findOne', () => {
    const fullInquiry = {
      ...
      listing: {
        ...
        currency: 'USD',
        photos: [],
        host: { id: HOST_ID, firstName: 'Host', phone: '+1555', whatsapp: '+1555' },
      },
      tenant: {
        id: TENANT_ID,
        firstName: 'Tenant',
    ...
    it('should hide sensitive address when not accepted', async () => {
      mockPrisma.inquiry.findUnique.mockResolvedValue(fullInquiry);
      const result = await service.findOne(INQUIRY_ID, TENANT_ID);
      expect((result.data.listing as Record<string, unknown>).streetAddress).toBeNull();
      expect((result.data.listing as Record<string, unknown>).zipCode).toBeNull();
    });

    it('should throw NotFoundException when not found', async () => {
      mockPrisma.inquiry.findUnique.mockResolvedValue(null);
      await expect(service.findOne(INQUIRY_ID, TENANT_ID)).rejects.toThrow(NotFoundException);
    });

    it('should throw ForbiddenException for unrelated user', async () => {
      mockPrisma.inquiry.findUnique.mockResolvedValue(fullInquiry);
      await expect(service.findOne(INQUIRY_ID, OTHER_ID)).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('accept', () => {
    it('should accept a pending inquiry', async () => {
      mockPrisma.inquiry.findUnique.mockResolvedValue(mockInquiry);
      mockPrisma.inquiry.update.mockResolvedValue({ ...mockInquiry, status: InquiryStatus.accepted });
    ...
    it('should throw ForbiddenException if not the host', async () => {
      mockPrisma.inquiry.findUnique.mockResolvedValue(mockInquiry);
      await expect(service.accept(INQUIRY_ID, OTHER_ID)).rejects.toThrow(ForbiddenException);
    });

    it('should throw BadRequestException if not pending', async () => {
      mockPrisma.inquiry.findUnique.mockResolvedValue({ ...mockInquiry, status: InquiryStatus.accepted });
      await expect(service.accept(INQUIRY_ID, HOST_ID)).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('reject', () => {
    it('should reject a pending inquiry', async () => {
      mockPrisma.inquiry.findUnique.mockResolvedValue(mockInquiry);
      mockPrisma.inquiry.update.mockResolvedValue({ ...mockInquiry, status: InquiryStatus.rejected });
      const result = await service.reject(INQUIRY_ID, HOST_ID, { reason: 'Not a good fit' });
      expect(result.data.status).toBe(InquiryStatus.rejected);
    });

    it('should throw ForbiddenException if not the host', async () => {
      mockPrisma.inquiry.findUnique.mockResolvedValue(mockInquiry);
      await expect(service.reject(INQUIRY_ID, OTHER_ID, {})).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('reply', () => {
    it('should add host reply', async () => {
      mockPrisma.inquiry.findUnique.mockResolvedValue(mockInquiry);
      mockPrisma.inquiry.update.mockResolvedValue({ ...mockInquiry, hostReply: 'Sure, come visit!' });
      const result = await service.reply(INQUIRY_ID, HOST_ID, { message: 'Sure, come visit!' });
    ...
    it('should throw BadRequestException when replying to a rejected inquiry', async () => {
      mockPrisma.inquiry.findUnique.mockResolvedValue({ ...mockInquiry, status: InquiryStatus.rejected });
      await expect(service.reply(INQUIRY_ID, HOST_ID, { message: 'text' })).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('cancel', () => {
    it('should cancel a pending inquiry', async () => {
      mockPrisma.inquiry.findUnique.mockResolvedValue(mockInquiry);
      mockPrisma.inquiry.update.mockResolvedValue({ ...mockInquiry, status: InquiryStatus.cancelled });
    ...
    it('should throw ForbiddenException if not the tenant', async () => {
      mockPrisma.inquiry.findUnique.mockResolvedValue(mockInquiry);
      await expect(service.cancel(INQUIRY_ID, OTHER_ID)).rejects.toThrow(ForbiddenException);
    });

    it('should throw BadRequestException if not pending', async () => {
      mockPrisma.inquiry.findUnique.mockResolvedValue({ ...mockInquiry, status: InquiryStatus.accepted });
      await expect(service.cancel(INQUIRY_ID, TENANT_ID)).rejects.toThrow(BadRequestException);
    });

    it('should throw NotFoundException when inquiry not found', async () => {
      mockPrisma.inquiry.findUnique.mockResolvedValue(null);
      await expect(service.cancel(INQUIRY_ID, TENANT_ID)).rejects.toThrow(NotFoundException);
    });
  });
});
