...
describe('ListingController', () => {
  ...
  describe('findMyListings', () => {
    it('should delegate to listingService.findMyListings', async () => {
      ...
      mockListingService.findMyListings.mockResolvedValue({ data: [], meta: {} });

      await controller.findMyListings(USER_ID, query as any);

      expect(mockListingService.findMyListings).toHaveBeenCalledWith(USER_ID, query);
    });
  });

  describe('findOne', () => {
    it('should delegate to listingService.findOne (public)', async () => {
      mockListingService.findOne.mockResolvedValue({ data: { id: LISTING_ID } });
  ...
  describe('update', () => {
    it('should delegate to listingService.update', async () => {
      ...
      expect(mockListingService.update).toHaveBeenCalledWith(LISTING_ID, USER_ID, dto);
    });
  });
  ...
  describe('remove', () => {
    it('should delegate to listingService.remove', async () => {
      ...
      expect(mockListingService.remove).toHaveBeenCalledWith(LISTING_ID, USER_ID);
    });
  });
  ...
  describe('publish', () => {
    it('should delegate to listingService.publish', async () => {
      mockListingService.publish.mockResolvedValue({ data: { status: 'pending_review' } });

      await controller.publish(LISTING_ID, USER_ID);

      expect(mockListingService.publish).toHaveBeenCalledWith(LISTING_ID, USER_ID);
    });
  });

  describe('pause', () => {
    it('should delegate to listingService.pause', async () => {
      mockListingService.pause.mockResolvedValue({ data: { status: 'paused' } });

      await controller.pause(LISTING_ID, USER_ID);

      expect(mockListingService.pause).toHaveBeenCalledWith(LISTING_ID, USER_ID);
    });
  });

  describe('resume', () => {
    it('should delegate to listingService.resume', async () => {
      mockListingService.resume.mockResolvedValue({ data: { status: 'active' } });

      await controller.resume(LISTING_ID, USER_ID);

      expect(mockListingService.resume).toHaveBeenCalledWith(LISTING_ID, USER_ID);
    });
  });

  describe('markRented', () => {
    it('should delegate to listingService.markRented', async () => {
      mockListingService.markRented.mockResolvedValue({ data: { status: 'rented' } });

      await controller.markRented(LISTING_ID, USER_ID);

      expect(mockListingService.markRented).toHaveBeenCalledWith(LISTING_ID, USER_ID);
    });
  });
  ...
  describe('addPhoto', () => {
    it('should delegate to listingService.addPhoto', async () => {
      const body = { url: 'http://img.com/1.jpg', thumbnailUrl: 'http://img.com/1-t.jpg', caption: 'Room' };
      mockListingService.addPhoto.mockResolvedValue({ data: { id: 'photo-1' } });

      await controller.addPhoto(LISTING_ID, USER_ID, body);

      expect(mockListingService.addPhoto).toHaveBeenCalledWith(
        LISTING_ID, USER_ID, body.url, body.thumbnailUrl, body.caption,
      );
    });
    ...
    it('should throw BadRequestException when url missing', () => {
      ...
      expect(() => controller.addPhoto(LISTING_ID, USER_ID, body)).toThrow(BadRequestException);
    });
  });
  ...
  describe('removePhoto', () => {
    it('should delegate to listingService.removePhoto', async () => {
      ...
      expect(mockListingService.removePhoto).toHaveBeenCalledWith(LISTING_ID, 'photo-1', USER_ID);
    });
  });
  ...
  describe('reorderPhotos', () => {
    it('should delegate to listingService.reorderPhotos', async () => {
      ...
      expect(mockListingService.reorderPhotos).toHaveBeenCalledWith(LISTING_ID, USER_ID, ['b', 'a']);
    });
  });
});
