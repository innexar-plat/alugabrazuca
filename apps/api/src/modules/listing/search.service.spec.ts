...
import { ListingStatus, ParkingType, PetPolicy, SmokingPolicy, ListingType } from '@prisma/client';
...
const mockListing = {
  ...
  photos: [{ url: 'https://example.com/photo.jpg', thumbnailUrl: 'https://example.com/thumb.jpg' }],
...
describe('SearchService', () => {
  ...
  describe('getCities', () => {
    it('should return grouped cities with counts', async () => {
      const groupByResult = [
        { country: 'USA', state: 'Florida', city: 'Orlando', _count: { id: 5 } },
        { country: 'Portugal', state: 'Lisboa', city: 'Lisboa', _count: { id: 3 } },
      ];
  ...
  describe('getSuggestions', () => {
    ...
    it('should return formatted suggestions when query has 2+ characters', async () => {
      ...
      expect(result.data).toEqual([
        ...
        { label: 'Orlando Park, Florida, USA', city: 'Orlando Park', state: 'Florida', country: 'USA' },
      ]);
        ...
        expect.objectContaining({
          where: expect.objectContaining({
            OR: expect.arrayContaining([
              expect.objectContaining({ city: { contains: 'Or', mode: 'insensitive' } }),
            ]),
          }),
        }),
      );
