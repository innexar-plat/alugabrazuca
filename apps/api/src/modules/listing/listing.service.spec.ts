...
describe("ListingService", () => {
  ...
  describe("findOne", () => {
    ...
    it("should throw NotFoundException for non-owner viewing draft listing", async () => {
      ...
      await expect(service.findOne(LISTING_ID, OTHER_ID)).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe("findMyListings", () => {
    it("should return paginated listings with meta", async () => {
      ...
      const result = await service.findMyListings(HOST_ID, {
        page: 1,
        limit: 20,
      });
  ...
  describe("update", () => {
    it("should update listing successfully", async () => {
      ...
      const result = await service.update(LISTING_ID, HOST_ID, {
        title: "Updated",
      } as any);
  ...
  describe("remove", () => {
    ...
    it("should throw ForbiddenException for non-owner", async () => {
      ...
      await expect(service.remove(LISTING_ID, OTHER_ID)).rejects.toThrow(
        ForbiddenException,
      );
    });
  });
  ...
  describe("publish", () => {
    ...
    it("should throw BadRequestException with fewer than 3 photos", async () => {
      ...
      await expect(service.publish(LISTING_ID, HOST_ID)).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("pause", () => {
    ...
    it("should throw BadRequestException for non-active listing", async () => {
      ...
      await expect(service.pause(LISTING_ID, HOST_ID)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("resume", () => {
    ...
    it("should throw BadRequestException for non-paused listing", async () => {
      ...
      await expect(service.resume(LISTING_ID, HOST_ID)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("markRented", () => {
    ...
    it("should throw BadRequestException for draft listing", async () => {
      ...
      await expect(service.markRented(LISTING_ID, HOST_ID)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("addPhoto", () => {
    it("should add photo with correct sort order", async () => {
      mockPrisma.listing.findFirst.mockResolvedValue(mockListing);
      mockPrisma.listingPhoto.aggregate.mockResolvedValue({
        _max: { sortOrder: 2 },
      });
  ...
  describe("removePhoto", () => {
    it("should delete photo successfully", async () => {
      mockPrisma.listing.findFirst.mockResolvedValue(mockListing);
      mockPrisma.listingPhoto.findFirst.mockResolvedValue({
        id: "photo-1",
  ...
  describe("reorderPhotos", () => {
    it("should reorder photos and return sorted list", async () => {
      ...
      const result = await service.reorderPhotos(LISTING_ID, HOST_ID, [
        "b",
        "a",
      ]);
